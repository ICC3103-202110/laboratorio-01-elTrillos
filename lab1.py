...
def cardgen(size):           
    ...
    #lo obtuve de stackoverflow, entrega los factores que al multiplicarse llegan al numero buscado
    alto = aa[-1]
    ancho = aa[-2]
    #los ultimos dos valores de la lista que genera siempre 
    # van a ser los que entreguen la matriz mas cuadrada posible
    #print(alto,ancho)
    while k1 <= size:
        listaOG.append(k1)
        listaOG.append(k1)
...
def gameplay():
    ...
    while playing == True:
        asking = True
        while asking == True:
            print("Player ", curplaya , "choose the card (x y) number", curpart)
            ans = input().split(" ")
